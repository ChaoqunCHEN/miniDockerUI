...
## 2026-02-24 (Wave 4: Fixture Orchestrator & Lifecycle Scenarios)
1. When creating Task closures in Swift 6 that capture `@Sendable` closures passed as function parameters, the closure must be bound to a local `let` and explicitly captured in the Task's capture list. Otherwise, Swift 6 flags a "passing closure as 'sending' parameter risks data races" error because the original parameter variable is still accessible in the calling scope.
2. The `DockerFixtureOrchestrator` pattern of using `docker ps -a --filter name=mdui-test-{runID}` for cleanup is reliable because Docker's name filter matches as a substring, so all containers for a run ID are captured in a single query.
3. Making `removeFixtures` completely error-swallowing (catching all exceptions, never rethrowing) ensures test cleanup is always best-effort and cannot itself cause test failures, which is critical for `tearDown` methods.
